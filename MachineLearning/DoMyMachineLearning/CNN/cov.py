...
class cov_2D_list:
    ...
    def cov_2D_pre(self,step=1, filter=filter, padding=0,bias=0):
        ...
        self.bias = bias
        self.matrix,self.matrix_padding,self.site_cov=cov_2D(matrix_one=self.matrix, step=self.step, filter=self.filter, padding=self.padding,bias=self.bias)
    ...
    def pooling(self, x_n=2, y_n=2, pooling_way='max', padding=2):
        ...
        if pooling_way=='max':
            ...
            self.matrix,self.x_long,self.y_long,self.site,self.pooling_padding= pooling_layer_2D(self.matrix,x_n=x_n, y_n=y_n, pooling_way='max', padding=padding)
        elif pooling_way == 'mean':

            self.matrix,self.x_long,self.y_long,self.site ,self.pooling_padding= pooling_layer_2D(self.matrix,x_n=x_n, y_n=y_n, pooling_way='mean', padding=padding)
class nn:
    def __init__(self,matrix=[]):
        ...
        self.Sequential=[]
        self.matrix_shape=[]
        self.Sequential_dic = {'cov_add':['self.cov_2D.matrix:卷积后的特征矩阵','self.cov_2D.matrix_padding:卷积前输入图像的预先补0扩张后的矩阵',
                                          'step：卷积移动步长','filter：卷积核','padding：补0的数量','no_cov_matrix:卷积前的输入矩阵',
                                          'self.cov_2D.site_cov:卷积过程中产生的映射前原来输入元素的坐标','bias:特特征图偏置'],

                               'act_add':['self.cov_2D.matrix:激活后的特征矩阵', 'activate_function:激活函数名', 'compare_num：如果是relu函数，那么就是比较值得阈值，否则为空'],

                                'pooling_add':[ 'self.cov_2D.matrix:池化后的特征矩阵','self.cov_2D.pooling_padding:池化前补0后的矩阵', 'self.cov_2D.x_long：池化块水平方向移动次数', 'self.cov_2D.y_long：垂直方向移动次数',
                                                'self.cov_2D.site：池化块原来最大值的相对位置', 'x_n：池化块的长', 'y_n：池化块的高', 'pooling_way：池化方式','pooling_padding_num:补数量']
                               }
    ...
    def cov_add(self,step=None, filter=None, padding=None,bias=None):
        ...
        self.Sequential.append(['cov_add',self.cov_2D.matrix,self.cov_2D.matrix_padding,step,filter,padding,no_cov_matrix,self.cov_2D.site_cov,bias])
    ...
    def pooling_add(self,x_n = None, y_n = None, pooling_way = None, pooling_padding_num = None):
        ...
        self.Sequential.append(['pooling_add',self.cov_2D.matrix,self.cov_2D.pooling_padding,self.cov_2D.x_long,self.cov_2D.y_long,self.cov_2D.site,x_n, y_n,pooling_way,pooling_padding_num])
